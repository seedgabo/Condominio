...
<script type="text/javascript">
	$(document).ready(function () {
		$('#table').jtable({
			...
			jqueryuiTheme: true,
			actions: {
				listAction:   "{{url('ajax/noticias/list')}}",
				createAction: "{{url('ajax/noticias/create')}}",
				updateAction: "{{url('ajax/noticias/edit')}}",
				deleteAction: "{{url('ajax/noticias/remove')}}",
			},
			fields: {
				id: {
					title: 'ID',
				...
				media:
				{
					...
					{
						if (data.record.media)
							return '<img src="{{asset("images/")}}/'+ data.record.media+'" height="48" width="48" />';
						else
							return "Ninguna";
					}
				},
